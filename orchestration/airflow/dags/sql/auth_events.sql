...
SELECT
    ...
    COALESCE(SAFE_CAST(JSON_VALUE(TO_JSON_STRING(src), '$.success') AS BOOL), FALSE) AS success
FROM {{ BIGQUERY_DATASET }}.{{ AUTH_EVENTS_TABLE}}_{{ logical_date.strftime("%m%d%H") }} AS src