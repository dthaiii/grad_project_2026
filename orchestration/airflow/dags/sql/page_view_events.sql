...
SELECT
    ...
    COALESCE(JSON_VALUE(TO_JSON_STRING(src), '$.page'), 'NA') AS page,
    COALESCE(JSON_VALUE(TO_JSON_STRING(src), '$.auth'), 'NA') AS auth,
    COALESCE(JSON_VALUE(TO_JSON_STRING(src), '$.method'), 'NA') AS method,
    COALESCE(SAFE_CAST(JSON_VALUE(TO_JSON_STRING(src), '$.status') AS INT64), 0) AS status,
